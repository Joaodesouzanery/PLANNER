...
const FinanceTransactions = () => {
  const { rawTransactions, allCategories, saveTransactionMutation, deleteTransactionMutation, selectedAccounts, accounts } = useFinanceWorkspace();
  const [showModal, setShowModal] = useState(false);
  const [editingTransaction, setEditingTransaction] = useState<Transaction | null>(null);
  const [filterCategory, setFilterCategory] = useState("");
  const [filterType, setFilterType] = useState("");
  const [form, setForm] = useState({ description: "", amount: 0, type: "income" as "income" | "expense", category: "", date: format(new Date(), "yyyy-MM-dd"), due_date: format(new Date(), "yyyy-MM-dd"), status: "confirmed", finance_account_id: "", is_recurring: false, recurrence_interval: "" });
  ...
  const handleSave = () => {
    ...
    saveTransactionMutation.mutate({ form: payload, editingId: editingTransaction?.id }, {
      onSuccess: () => {
        ...
        setForm({ description: "", amount: 0, type: "income", category: "", date: format(new Date(), "yyyy-MM-dd"), due_date: format(new Date(), "yyyy-MM-dd"), status: "confirmed", finance_account_id: "", is_recurring: false, recurrence_interval: "" });
      },
    });
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => { setForm((current) => ({ ...current, finance_account_id: current.finance_account_id || selectedAccounts[0]?.id || "" })); setShowModal(true); }} className="rounded-xl shadow-lg shadow-primary/20"><Plus className="h-4 w-4 mr-2" />Nova Transação</Button>
        </div>
      </div>
      <Card className="border border-border/50 bg-card/80">
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table>
              <TableHeader><TableRow><TableHead>Data / vencimento</TableHead><TableHead>Descrição</TableHead><TableHead>Conta</TableHead><TableHead>Categoria</TableHead><TableHead>Tipo</TableHead><TableHead className="text-right">Valor</TableHead><TableHead></TableHead></TableRow></TableHeader>
              <TableBody>
                  ...
                  <TableRow key={t.id} className="hover:bg-muted/30">
                    ...
                    <TableCell>
                      <div className="flex gap-1 justify-end">
                        <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-primary/10 hover:text-primary" onClick={() => { setEditingTransaction(t); setForm({ description: t.description, amount: t.amount, type: t.type, category: t.category || "", date: t.date, due_date: t.due_date || t.date, status: t.status || "confirmed", finance_account_id: t.finance_account_id || "", is_recurring: t.is_recurring || false, recurrence_interval: t.recurrence_interval || "" }); setShowModal(true); }}><Edit2 className="h-4 w-4" /></Button>
                ...
                {filteredTransactions.length === 0 && <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground italic">Nenhuma transação encontrada.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </div>
        </CardContent>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div><Label>Status</Label><Select value={form.status} onValueChange={value => setForm({ ...form, status: value })}><SelectTrigger className="rounded-xl"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="planned">Falta pagar/receber</SelectItem><SelectItem value="confirmed">Confirmado</SelectItem><SelectItem value="reconciled">Conciliado</SelectItem></SelectContent></Select></div>
            </div>
