    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(240px,1fr)] gap-4">
      ...
      <Card className="h-fit">
        ...
        <CardContent className="space-y-2">
          ...
          <div className="rounded-lg border border-border/50 p-2.5"><p className="text-[11px] text-muted-foreground">Deals abertos</p><p className="font-mono text-lg font-bold">{(crm.deals as any[]).filter((d) => !["ganho", "perdido", "won", "lost"].includes((d.stage || "").toLowerCase())).length}</p></div>
        </CardContent>
      </Card>
    </div>
  );
