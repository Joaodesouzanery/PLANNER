...
import {
  BarChart3, Building2, Target, DollarSign, TrendingUp, GitCompareArrows,
} from "lucide-react";
import {
...
const CommercialComparison = () => {
  ...
  const companyStructures: CompanyStructure[] = companies
    .map((company) => {
      ...
      rows.forEach((r: any) => { sections[r.section] = r.content; });
  ...
  const funnelComparisonData = () => {
    ...
    return Array.from({ length: maxSteps }, (_, i) => {
      const point: any = { step: `Etapa ${i + 1}` };
      selected.forEach((cs) => {
  ...
  const channelRadarData = () => {
    ...
    return Array.from(allChannels).slice(0, 8).map((channel) => {
      const point: any = { channel };
      selected.forEach((cs) => {
        const b2g = cs.sections.playbook_b2g as PlaybookData | undefined;
        const b2b = cs.sections.playbook_b2b as PlaybookData | undefined;
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 p-4 sm:p-6">
        <div>
          <h1 className="text-2xl font-heading font-bold text-foreground flex items-center gap-2">
            <GitCompareArrows className="h-6 w-6 text-primary" /> Comparativo Comercial
          </h1>
            ...
            {companyStructures.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-3">
                {companyStructures.map((cs, i) => (
                  <label key={cs.company.id} className="flex items-center gap-2 cursor-pointer p-2 rounded-lg border border-border hover:bg-muted/50 transition-colors">
                    <Checkbox
                      checked={selectedIds.includes(cs.company.id)}
                      onCheckedChange={() => toggleCompany(cs.company.id)}
                    />
                    <div className="flex items-center gap-1.5">
                      <div className={`h-3 w-3 rounded-full`} style={{ backgroundColor: COLORS[i % COLORS.length] }} />
                      <span className="text-sm font-medium">{cs.company.name}</span>
                    </div>
        ...
        {selected.length >= 2 && (
          <Tabs defaultValue="funnel">
            <TabsList className="w-full flex-wrap h-auto gap-1 p-1">
              ...
              <TabsTrigger value="channels" className="flex items-center gap-1.5"><TrendingUp className="h-3.5 w-3.5" />Canais</TabsTrigger>
              <TabsTrigger value="commission" className="flex items-center gap-1.5"><DollarSign className="h-3.5 w-3.5" />Comissão</TabsTrigger>
              <TabsTrigger value="detail" className="flex items-center gap-1.5"><Building2 className="h-3.5 w-3.5" />Detalhes</TabsTrigger>
            </TabsList>

            <TabsContent value="funnel" className="mt-4">
              <Card>
                <CardHeader><CardTitle className="text-base">Cobertura do Funil de Vendas</CardTitle></CardHeader>
                <CardContent>
                  <div className="h-[350px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={funnelComparisonData()}>
                        ...
                        <Legend />
                        {selected.map((cs, i) => (
                          <Bar key={cs.company.id} dataKey={cs.company.name} fill={COLORS[i % COLORS.length]} />
                        ))}
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
              ...
              {/* Funnel detail table */}
              <div className="grid gap-4 mt-4" style={{ gridTemplateColumns: `repeat(${selected.length}, 1fr)` }}>
                {selected.map((cs, i) => (
                  <Card key={cs.company.id} className="border-t-2" style={{ borderTopColor: COLORS[i % COLORS.length] }}>
                    <CardHeader className="pb-2"><CardTitle className="text-sm">{cs.company.name}</CardTitle></CardHeader>
            ...
            <TabsContent value="kpis" className="mt-4">
              <Card>
                <CardHeader><CardTitle className="text-base">Quantidade de KPIs por Papel</CardTitle></CardHeader>
                <CardContent>
                  <div className="h-[350px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={kpiComparisonData()}>
                        ...
                        <Tooltip />
                        <Legend />
                        <Bar dataKey="KPIs BDR" fill="#3b82f6" />
                        <Bar dataKey="KPIs AE" fill="#8b5cf6" />
                        <Bar dataKey="KPIs CSM" fill="#10b981" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
            ...
            <TabsContent value="channels" className="mt-4">
              <Card>
                <CardHeader><CardTitle className="text-base">Canais de Prospecção (Radar)</CardTitle></CardHeader>
                <CardContent>
                  <div className="h-[400px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart data={channelRadarData()}>
                        ...
                        {selected.map((cs, i) => (
                          <Radar key={cs.company.id} name={cs.company.name} dataKey={cs.company.name}
            ...
            <TabsContent value="commission" className="mt-4">
              <Card>
                <CardHeader><CardTitle className="text-base">Estrutura de Comissionamento</CardTitle></CardHeader>
                <CardContent>
                  <div className="h-[350px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={commissionComparisonData()}>
                        ...
                        <Tooltip />
                        <Legend />
                        <Bar dataKey="Papéis" fill="#f59e0b" />
                        <Bar dataKey="Itens de Comissão" fill="#8b5cf6" />
                        <Bar dataKey="Considerações" fill="#10b981" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
                    ...
                    <Card key={cs.company.id} className="border-t-2" style={{ borderTopColor: COLORS[i % COLORS.length] }}>
                      ...
                      <CardContent className="space-y-3 text-xs">
                        ...
                        <div>
                          ...
                          {funnel.map((s, j) => (
                            <p key={j} className="text-muted-foreground">{s.step}. {s.title} ({s.responsible})</p>
                          ))}
                        </div>
                      </CardContent>
                    </Card>
